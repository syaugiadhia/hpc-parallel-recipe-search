...
// nameId = id elemen (RecipeGraph::idOf). Mengganti std::string name supaya
// CompactNode trivially-copyable -> copy FrontierNode jadi memcpy POD (bukan
// alokasi string per node). Ini inti optimasi paralel: copy jauh lebih murah,
// RAM turun, dan ekspansi tidak lagi memory-bandwidth-bound.
struct CompactNode {
    int nameId = -1;
    int left = -1;
    int right = -1;
...
bool isExpandableCompactLeaf(const RecipeGraph& graph, const CompactNode& node) {
    if (node.left >= 0 || node.right >= 0 || node.basic || node.nameId < 0) {
        return false;
    }
    const std::string& name = graph.nameOfId(node.nameId);
    return graph.hasElement(name) && !graph.isTerminal(name) && graph.hasRecipes(name);
}
...
int appendCompactTree(const RecipeGraph& graph,
    ...
    CompactNode node;
    node.nameId = graph.idOf(canonical);
    node.parent = parent;
...
bool pathContainsId(const std::vector<CompactNode>& nodes, int leafIndex, int targetId) {
    for (int index = leafIndex; index >= 0; index = nodes[index].parent) {
        if (nodes[index].nameId == targetId) {
            return true;
        }
    }
    return false;
...
RecipeTree compactToRecipeTree(const RecipeGraph& graph, const std::vector<CompactNode>& nodes, int index) {
    const auto& node = nodes[index];
    RecipeTree tree;
    tree.name = graph.nameOfId(node.nameId);
    tree.basic = node.basic;
    if (node.left >= 0 && node.right >= 0) {
        tree.children.push_back(compactToRecipeTree(graph, nodes, node.left));
        tree.children.push_back(compactToRecipeTree(graph, nodes, node.right));
    }
    return tree;
}
...
std::string compactSignature(const std::vector<CompactNode>& nodes, int index) {
    ...
    std::string signature = std::to_string(node.nameId) + "(";
...
FrontierExpansion expandFrontierState(const RecipeGraph& graph,
    ...
    const std::string& canonicalLeaf = graph.nameOfId(current.nodes[leafIndex].nameId);
    output.nodesVisited = 1;
    for (const auto& recipe : graph.recipesFor(canonicalLeaf)) {
        const int firstId = graph.idOf(recipe.first);
        const int secondId = graph.idOf(recipe.second);
        if (pathContainsId(current.nodes, leafIndex, firstId) ||
            pathContainsId(current.nodes, leafIndex, secondId)) {
...
SearchEngine::ExpandResult SearchEngine::expandPartialBfsLazy(const RecipeTree& partial, int limit) {
    ...
    while (!frontier.empty() && !limitReached(result.trees.size(), limit)) {
        ...
        const std::string& canonicalLeaf = graph_.nameOfId(current.nodes[leafIndex].nameId);
        ++stats_.nodesVisited;
        maybePrintNodeProgress();

        for (const auto& recipe : graph_.recipesFor(canonicalLeaf)) {
            const int firstId = graph_.idOf(recipe.first);
            const int secondId = graph_.idOf(recipe.second);
            if (pathContainsId(current.nodes, leafIndex, firstId) ||
                pathContainsId(current.nodes, leafIndex, secondId)) {
                result.cycleBlocked = true;
                continue;
...
SearchEngine::ExpandResult SearchEngine::expandPartialBfsLazyOpenmp(const RecipeTree& partial, int limit) {
    ...
    while (!frontier.empty() && !limitReached(result.trees.size(), limit)) {
        ...
        // Hanya fork/join tim thread kalau batch cukup besar untuk menutup overheadnya.
        // Di workload besar, banyak level frontier punya batch kecil; fork/join per level
        // (dulu tanpa syarat) bikin openmp lebih lambat dari serial. Output sama persis
        // (ekspansi identik, cuma dihitung serial saat batch kecil).
        if (static_cast<int>(batch.size()) >= 2 * threadCount) {
